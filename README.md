...
Sheng Game Framework 是一个面向 Unity 项目的轻量基础框架，当前提供单例、事件管理、计时器、场景管理、AssetBundle、对象池、UI、状态机、行为树、JSON 数据管理、Luban 配置、多平台构建以及 UnityAgentBridge 自动化能力
...
| TimerManager | 延迟、循环、暂停、时间缩放和场景生命周期 | Runtime |
| SceneManager | 串行异步加载、进度、Additive、卸载和生命周期事件 | Runtime |
...
| Framework Updater | GitHub 新版本检查、更新确认和 UPM 自动解析 | Editor |
...
flowchart LR
    ...
    Agent --> Tests["项目诊断与测试"]
    Updater["Framework Updater"] --> Assets
```
...
| [TimerManager](Docs/TimerManager.md) | 延迟、循环、暂停、时间类型和生命周期 |
| [SceneManager](Docs/SceneManager.md) | 异步加载、进度、队列、Additive、卸载和生命周期事件 |
...
| [框架更新器](Docs/Framework_Updater.md) | 自动检查、手动检查和 UPM 更新流程 |
...
- 已支持外部传入 `deltaTime` 驱动计时器、状态机和行为树
- 尚未实现定点数、确定性物理、输入帧缓存、回滚或网络帧同步
- AssetManager 尚未实现远程下载、版本校验和热更新流程
- SceneManager 当前只加载 Build Settings 场景，不支持 Addressables 或远程 AB 场景
