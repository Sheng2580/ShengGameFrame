...
| `GetLubanStatus` | 查询 Luban 环境和配置类扫描状态 |
| `StartLubanInstallation` | 异步安装固定版本 Luban |
| `ValidateLubanProject` | 检查 C# 配置类和 Excel 表结构 |
| `StartLubanValidation` | 异步调用 Luban 校验配置表 |
| `StartLubanJsonGeneration` | 异步调用 Luban 生成 JSON |
| `GetTaskStatus` | 查询最近构建、Luban 安装、校验或生成任务 |
