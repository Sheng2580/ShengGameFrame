...
核心类型：`AssetManager`、`AssetManagerSettings`、`AssetBundlePath`
...
```csharp
AudioClip battleMusic = AssetManager.Instance.LoadAsset<AudioClip>(
    "audio",
    "BattleMusic");
...
```csharp
AssetManager.Instance.ReleaseAsset(battleMusic);
battleMusic = null;
```

业务层只接触真实资源对象，不接触引用句柄。每次成功调用 `LoadAsset` 都增加一次资源引用，因此同一资源加载多次时也要对应释放多次
...
AssetManager.Instance.LoadAssetAsync<AudioClip>(
    ...
    failed: () => Debug.LogError("BattleMusic 加载失败"));
```

使用完成后调用 `ReleaseAsset(battleMusic)`。加载失败时 `AssetManager` 会统一输出资源路径和类型，成功回调不会收到 `null`。只有业务需要额外处理失败时才传入 `failed` 回调

只想提前加载一个 `GameObject` 到缓存时，可以沿用最简单的无参数回调：
...
// 不再需要预加载资源时释放
AssetManager.Instance.ReleaseAsset<GameObject>("obj", "cube");
```

相同资源的并发请求会合并。全部成功回调收到同一个 Unity 资源对象，但每次请求仍分别持有一次资源引用
...
```csharp
GameObject enemy = AssetManager.Instance.Instantiate(
    "characters",
    "Enemy");
```
...
```csharp
AssetManager.Instance.ReleaseInstance(enemy);
enemy = null;
```

实例内部带有不可见的生命周期追踪组件，运行时直接调用 `Destroy(enemy)` 也会自动释放预制体引用。推荐优先调用 `ReleaseInstance`，资源释放会更及时

实例要交给其他系统继续管理时，可以只释放预制体引用并保留对象：

```csharp
AssetManager.Instance.ReleaseInstance(enemy, destroyInstance: false);
```
...
再次加载 -> Asset 引用 +1
ReleaseAsset -> Asset 引用 -1
Instantiate -> 预制体 Asset 引用 +1
ReleaseInstance 或实例销毁 -> 预制体 Asset 引用 -1
...
AssetManager.Instance.LoadAssetAsync<Texture2D>(
    "ui",
    "CommonIcon",
    texture => AssetManager.Instance.ReleaseAsset(texture),
    cachePolicy: AssetCachePolicy.KeepLoaded);
```
...
| `LoadAsset<T>` | 同步加载并返回资源对象 |
| `LoadAssetAsync<T>` | 异步加载并返回资源对象 |
| `Instantiate` | 同步加载并实例化预制体 |
| `InstantiateAsync` | 异步加载并实例化预制体 |
| `ReleaseAsset` | 释放一次资源引用 |
| `ReleaseInstance` | 释放实例及其预制体引用 |
...
- `PoolManager` 会在资源池存续期间持有预制体资源引用，并在删池完成时自动释放，详见[对象池模块](PoolManager.md)
